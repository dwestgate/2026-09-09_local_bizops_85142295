...
## The models stage: the one manual step in this release

On a fresh Mac the preview stops at `models` with `error_kind:
...
discovered from a running LM Studio server, and **this release has no
...
1. Install LM Studio's headless service, **pinned to `llmster 0.0.23-1`**.
   ...
   **Do not run the plain `curl … | sh` one-liner unpinned.** As of
   ...
   at `0.00%` for more than two attempts, stop retrying, delete the
   `.part` file under `~/.lmstudio/models/`, apply the pin, and start clean.
...
4. Load both and prove they are served. `~/.lmstudio/bin/lms ls` must show
   ...
   conservative. `curl -s http://localhost:1234/v1/models` then lists three
   ids, two of which differ by one suffix: pick
...
6. Make the server survive a reboot. In LM Studio's settings enable the
   ...
   exact wording varies by version. Nothing this release installs does that
