...
def inference_qualification(request: AdapterRequest, runtime: Runtime) -> JsonObject:
    ...
    try:
        ...
        return _boolean_probe(
            ...
            observed=False,
            source=source,
            repair=_inference_repair(candidate, None, timed_out=True, valid_action=False),
    ...
    valid_action = _valid_structured_action(body)
    valid = (
        status == 200
        and returned_candidate == candidate
        and valid_action
...
def _inference_repair(
    ...
    *,
    ...
    if not valid_action:
        return (
            "Inference response did not contain the required structured action. Repair the "
            "model's reviewed structured-output response and retry setup."
        )
