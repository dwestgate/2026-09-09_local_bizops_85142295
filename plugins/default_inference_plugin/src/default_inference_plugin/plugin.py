...
class Plugin(
    ...
    def prepare_for_readiness(self) -> None:
        ...
        # A configured but not-yet-started LM Studio must not make the whole
        # platform crash-loop. Preserve an explicit error state — callers are
        # guarded in generate_completion() — while a rate-bounded worker waits
        # for the operator-managed service to become available.
        result = self.validate_availability()
        if result.get("action_status") == ActionStatus.ERROR.value:
            error_message = f"LM Studio not available at {lm_studio_url}"
            self.set_error(error_message)
            self.logger.warning("LM Studio not reachable at %s — waiting", lm_studio_url)
        ...
        try:
            ...
            error_message = f"LM Studio pre-warm failed: {exc}"
            self.set_error(error_message)
